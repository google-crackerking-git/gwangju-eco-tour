...
async function run() {
  ...
  const gwangjuItems = allItems.filter(item => 
    (item.beginRdnmadr && item.beginRdnmadr.includes('광주')) ||
    (item.beginLnmadr && item.beginLnmadr.includes('광주')) ||
    (item.insttNm && item.insttNm.includes('광주'))
  );
  console.log('Total Gwangju Paths:', gwangjuItems.length);
