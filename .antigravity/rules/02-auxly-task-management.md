...
const todoTasks = await mcp_auxly_auxly_list_tasks({ 
  status: "todo" 
});

// STEP 2: Check IN_PROGRESS tasks
const inProgressTasks = await mcp_auxly_auxly_list_tasks({ 
  status: "in_progress" 
});
...
### ❌ FORBIDDEN:
- Creating tasks without calling `mcp_auxly_auxly_list_tasks` first
...
### ❌ FORBIDDEN:
- Starting work without calling `mcp_auxly_auxly_get_task`
...
// STEP 1: Get task
const task = await mcp_auxly_auxly_get_task({ taskId: "14" });
...
**ENFORCEMENT: ALWAYS call `mcp_auxly_auxly_list_tasks` before creating, ALWAYS call `mcp_auxly_auxly_get_task` before starting work.**
